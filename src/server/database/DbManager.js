...
class DbManager {
    ...
    static db_name = 'redchan-main';
    static db_url = 'mongodb+srv://moxxos:l5iW8jILf93Toekn@redchan-main.aa6qv.mongodb.net/?retryWrites=true&w=majority&appName=redchan-main';
    mc; // MongoDB server connection
    db; // redchan database
    buckets;
    constructor() {
        this.mc = new mongodb_1.MongoClient(DbManager.db_url);
    ...
    async initDB(boards) {
        ...
        for (let board of boards) {
            ...
            await this.db.collection('counters').updateOne({ counter: `${board}_thread_id` }, { $setOnInsert: { sequence_value: 0 } }, { upsert: true });
            console.log(`Created ${board}_thread_id sequence document.`);
            await this.db.collection('counters').updateOne({ counter: `${board}_post_id` }, { $setOnInsert: { sequence_value: 0 } }, { upsert: true });
            console.log(`Created ${board}_post_id sequence document.`);
            this.buckets[board] = new mongodb_1.GridFSBucket(this.db, { bucketName: `${board}_uploads` });
    ...
    */
    ...
    async getThread(board, id, method = "thread_id") {
    ...
    */
    async getPagedThreads(board, page_num) {
        const threads = this.db.collection(`${board}_threads`);
    ...
    async getAllThreadPosts(board, thread_id) {
        const posts = this.db.collection(`${board}_posts`);
    ...
    async getPreviewThreadPosts(board, thread_id) {
        const posts = this.db.collection(`${board}_posts`);
        let results = await posts.find({ thread_id: thread_id }).sort({ post_id: -1 }).limit(5).toArray();
        return results;
    }
    async retrieveFile(board, id) {
        const file_id = new mongodb_1.ObjectId(id);
    ...
    uploadFile(name, type, size, data, board) {
        return new Promise((resolve, reject) => {
            ...
            const buffer_stream = new stream_1.Readable();
    ...
    async createThread(name, date_str, subject, comment, file_id, board, post_id, thread_id) {
        const threads = this.db.collection(`${board}_threads`);
        const new_thread = {
            date: date_str,
            comment: comment,
    ...
    async createPost(name, date_str, comment, file_id, board, thread_id, post_id) {
        const posts = this.db.collection(`${board}_posts`);
        let new_post = {
            date: date_str,
            comment: comment,
    ...
    async updatePostRefs(board, post_id, ref_id) {
        const posts = this.db.collection(`${board}_posts`);
    ...
    async updateThreadRefs(board, thread_id, ref_id) {
        const threads = this.db.collection(`${board}_threads`);
    ...
    async updateBumpId(board, thread_id, bump_id) {
        const threads = this.db.collection(`${board}_threads`);
        let result = await threads.updateOne({ thread_id: thread_id }, { $set: { bump_id: bump_id } });
        if (result.acknowledged) {
    ...
    // Delete thread
    async deleteThread(board, thread_id) {
        const threads = this.db.collection(`${board}_threads`);
    ...
    // Get IDs
    async getNextThreadId(board) {
        const counters = this.db.collection('counters');
        let result = await counters.findOneAndUpdate({ counter: `${board}_thread_id` }, { $inc: { sequence_value: 1 } }, { returnDocument: 'after' });
        if (result) {
    ...
    async getNextPostId(board) {
        const counters = this.db.collection('counters');
        let result = await counters.findOneAndUpdate({ counter: `${board}_post_id` }, { $inc: { sequence_value: 1 } }, { returnDocument: 'after' });
        if (result) {
            return result.sequence_value;
        }
        else {
